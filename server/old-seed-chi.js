...
db.sync({
  ...
  .then(function () {
    ...
    return Promise.all(Object.keys(data).map( function (name) {
      return Promise.all(data[name], function (item) {
        if (name === 'activity' || name === 'restaurant' || name === 'hotel') {
  ...
  .then( (instances) => {
    hotel = instances[0][2]
    restaurant = instances[1][4]
    activity = instances[2][6]
  ...
  .then( function(itinerary){
    ...
    return Promise.all([itinerary.addHotel(hotel), itinerary.addRestaurant(restaurant), itinerary.addActivity(activity)])
  })
  .then(function () {
